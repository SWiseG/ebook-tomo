...
- ✅ **Fase 7 — Auditoria de conversão por IA (WS-F; "linter de persuasão")** — novo purpose `ebook.audit` + prompt `prompts/ebook/audit.md`: a IA pontua o manuscrito + headline contra o checklist de conversão (promessa/4 U's, valor na 1ª página, hook por capítulo, PAS, prova social, micro-CTA, CTA final) e devolve **veredito** (pass/warn/fail) + **score** + itens. Query/handler (`GetConversionAuditQuery`, lê manuscrito/sales-copy, trunca a 16k chars, cacheado pelo AI Gateway), endpoint `GET /products/{id}/audit`, action "Auditar conversão" + dialog na product-detail (badge de veredito, score, checklist com ✓/✗) + i18n. **Gate consultivo** (mostra, não bloqueia). Verificado: parse do DTO + build Angular.
- ✅ **Fase 4 — Diretor de Arte por IA (WS-B; "toda revisão passa pela IA")** — novo purpose `ebook.visual-plan` + prompt `prompts/ebook/visual-plan.md`: a IA lê o **outline** e decide, por capítulo, **foto vs ilustração** + **query/prompt concretos** (anti-clichê). `PdfJobHandler` injeta `IAiGateway`, monta o plano (cacheado), casa por título (`ChapterKey`, tolerante ao prefixo "Capítulo N") e gera cada imagem guiada pela diretriz. `MediaBrief.Kind` (`Photo`/`Illustration`/`Auto`) + **priorização no `MediaGateway`** roteiam a cadeia conforme a decisão da IA (piso Skia sempre por último). Falha da IA/parse → cai nos prompts genéricos (não quebra). Verificado: roteamento por tipo (2 testes) + Pdf job end-to-end com plano (FakeAiGateway). **Pendente E15-04**: realimentar o plano com o style playbook aprendido (Onda 3).
